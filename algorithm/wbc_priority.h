...
//	void printInipos(const DataBus& robotState);
...
private:
	...
	Eigen::Vector3d curV_W; // 当前步速
	float history_size;       // 历史数据缓冲区大小
	std::deque<double> l_hip_q_hist;
	std::deque<double> r_hip_q_hist;
	std::deque<double> l_hip_dq_hist;
	std::deque<double> r_hip_dq_hist;
