...
MPC::MPC(double dtIn):QP(nu*ch, nc*ch) {
	...
	max[0] = 1000.0;  max[1] = 1000.0; max[2] = -3.0*m*g; // Fx, Fy, Fz 最大值. Fz用-m*g表示反作用力方向
	max[3] = 20.0;    max[4] = 80.0;   max[5] = 100.0;   // Mx, My, Mz 最大值

	min[0] = -1000.0; min[1] = -1000.0; min[2] = 0.0;     // Fx, Fy, Fz 最小值. Fz最小为0，表示脚不能拉地面
	min[3] = -20.0;   min[4] = -80.0;  min[5] = -100.0;   // Mx, My, Mz 最小值
	...
	// --- QP约束矩阵初始化 ---
	u_low.setZero(); u_up.setZero(); // 决策变量的上下界
	As.setZero();                   // 约束矩阵 A
...
void MPC::set_weight(double u_weight, Eigen::MatrixXd L_diag, Eigen::MatrixXd K_diag) {
	...
	for (int i = 0; i < mpc_N; i++){
		L.block<3,3>(i*nx + 3,i*nx + 3) = R_curz[i]*L.block<3,3>(i*nx + 3,i*nx + 3)*R_curz[i].transpose(); // 位置权重
		L.block<3,3>(i*nx + 6,i*nx + 6) = R_curz[i]*L.block<3,3>(i*nx + 6,i*nx + 6)*R_curz[i].transpose(); // 角速度权重
		L.block<3,3>(i*nx + 9,i*nx + 9) = R_curz[i]*L.block<3,3>(i*nx + 9,i*nx + 9)*R_curz[i].transpose(); // 线速度权重
	}
	// 同理，旋转控制输入的权重
	for (int i = 0; i < ch; i++){
...
void MPC::dataBusRead(DataBus &Data) {
	// --- 1. 读取并设置当前状态 X_cur ---
	// 状态向量X_cur: [roll, pitch, yaw, px, py, pz, wx, wy, wz, vx, vy, vz]' (12x1)
...
void MPC::cal() {
	if (EN) { // 只有在MPC启用时才执行计算
		...
		for (int i = 0; i < mpc_N; i++) {
			...
			Ac[i].block<3, 3>(3, 9) = Eigen::MatrixXd::Identity(3,3); // 线速度 -> 位置变化
			// 离散化: A = I + dt*Ac
		...
		for (int i = 0; i < mpc_N; i++) {
			...
			Bc[i].block<3, 3>(6, 6) = Ic_W_inv * CrossProduct_A(pf2comi[i].block<3, 1>(3, 0)); // 右脚反作用力产生的力矩
			Bc[i].block<3, 3>(6, 9) = Ic_W_inv;                                                 // 右脚反作用力矩
			// 力对线速度的影响
			Bc[i].block<3, 3>(9, 0) = Eigen::MatrixXd::Identity(3,3)/ m; // 左脚反作用力
			Bc[i].block<3, 3>(9, 6) = Eigen::MatrixXd::Identity(3,3)/ m; // 右脚反作用力
		...
		H = 2 * (Bqp.transpose() * L * Bqp + alpha * K) + 1e-10*Eigen::MatrixXd::Identity(nu*ch, nu*ch); // 加一个小的正则项防止H奇异
		...
		Asfr111.setZero(); Asfr1.setZero(); Asfr.setZero();
		...
		gxyz[0] << 0.0, 1.0, 0.0; gxyz[1] << 0.0, 1.0, 0.0; gxyz[2] << 1.0, 0.0, 0.0; gxyz[3] << 1.0, 0.0, 0.0;
		Eigen::Matrix<double, 3, 1> r[4]; Eigen::Matrix<double, 3, 1> p[4];
		Eigen::Matrix<double, ncstxya, 6> Astxy_r[4]; Eigen::Matrix<double, ncstxy_single, 6> Astxy11;
		Eigen::Matrix<double, ncstxy, nu> Astxy1; Eigen::Matrix<double, ncstxy * ch, nu * ch> Astxy;
		Astxy_r[0].setZero(); Astxy_r[1].setZero(); Astxy_r[2].setZero(); Astxy_r[3].setZero();
		Astxy11.setZero(); Astxy1.setZero(); Astxy.setZero();
		r[0] << 0.0, 1.0, 0.0; r[1] << 0.0, 1.0, 0.0; r[2] << 1.0, 0.0, 0.0; r[3] << 1.0, 0.0, 0.0;
		p[0] << delta_foot[0], 0.0, 0.0; p[1] << -delta_foot[1], 0.0, 0.0;
		p[2] << 0.0, delta_foot[2], 0.0; p[3] << 0.0, -delta_foot[3], 0.0;
		for (int i = 0; i < 4; i++) {
		...
		Astxy1.block<ncstxy_single, 6>(0, 0) = Astxy11; Astxy1.block<ncstxy_single, 6>(ncstxy_single, 6) = Astxy11;
		for (int i = 0; i < ch; i++)
			Astxy.block<ncstxy, nu>(ncstxy * i, nu * i) = Astxy1;

		// 4.2.2 Z方向力矩约束
		Eigen::Matrix<double, ncstza, 6> Astz_r[4]; Eigen::Matrix<double, ncstz_single, 6> Astz11;
		Eigen::Matrix<double, ncstz, nu> Astz1; Eigen::Matrix<double, ncstz * ch, nu * ch> Astz;
		Astz_r[0].setZero(); Astz_r[1].setZero(); Astz_r[2].setZero(); Astz_r[3].setZero();
		...
		Astz1.block<ncstz_single, 6>(0, 0) = Astz11; Astz1.block<ncstz_single, 6>(ncstz_single, 6) = Astz11;
		for (int i = 0; i < ch; i++)
		...
		for (int i = 0; i < ch; i++){
			if (legState[i] == DataBus::DSt) { // 双支撑
				...
				// 设置双脚力和力矩的上下限
				for (int j = 0; j < 6; j++) {
					u_low(i * nu + j) = min[j]; u_low(i * nu + j + 6) = min[j];
					u_up(i * nu + j) = max[j];  u_up(i * nu + j + 6) = max[j];
				}
				...
				for (int j = 0; j < 6; j++) {
					u_low(i * nu + j) = min[j]; u_low(i * nu + j + nu / 2) = 0.0;
					u_up(i * nu + j) = max[j];  u_up(i * nu + j + nu / 2) = 0.0;
				}
				...
				for (int j = 0; j < 6; j++) {
					u_low(i * nu + j) = 0.0;    u_low(i * nu + j + nu / 2) = min[j];
					u_up(i * nu + j) = 0.0;     u_up(i * nu + j + nu / 2) = max[j];
				}
			}
		}
